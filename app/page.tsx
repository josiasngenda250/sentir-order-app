...
function unitPrice(code: ProductCode){ return code==="TRIO" ? 77 : 30; }
...
export default function Page(){
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>){
    e.preventDefault();
    setMsg("");
    if(!canSubmit){ setMsg("Please complete all required selections."); return; }
    ...
    const body = {
      ...
      quantity: qty,
      shippingOption: shipping==="calgary" ? "Ship within Calgary — Free"
        : (shipping==="standard" ? "Standard Canada Post — $14 CAD (3–7 business days)"
  ...
  return (
    <div className="card">
      <p className="text-gray-700 mb-4">Each candle is crafted fresh when you order, with premium soy wax and clean‑burning wicks. <strong>No tax charges.</strong></p>

      <form onSubmit={submit}>
        {/* Customer */}
        <section className="mb-6">
          <h2 className="text-lg font-bold mb-2">Customer details</h2>
          <div className="field"><label className="label">Full name*</label><input className="input" name="fullName" required placeholder="Your name" /></div>
          <div className="grid md:grid-cols-2 gap-3">
            <div className="field"><label className="label">Email*</label><input className="input" name="email" type="email" required placeholder="you@example.com" /></div>
            <div className="field"><label className="label">Phone</label><input className="input" name="phone" type="tel" placeholder="(optional)" /></div>
          </div>
          <div className="field">
          ...
          <div className="field"><label className="label">Address line 1*</label><input className="input" name="addr1" required placeholder="Street address" /></div>
          <div className="field"><label className="label">Address line 2</label><input className="input" name="addr2" placeholder="Apartment, suite (optional)" /></div>
          <div className="grid md:grid-cols-3 gap-3">
            <div className="field"><label className="label">City*</label><input className="input" name="city" required /></div>
            <div className="field"><label className="label">Province/State*</label><input className="input" name="province" required /></div>
            <div className="field"><label className="label">Postal code*</label><input className="input" name="postal" required /></div>
          </div>
          <div className="field"><label className="label">Country*</label><input className="input" name="country" defaultValue="Canada" required /></div>
        </section>
        ...
        <section className="mb-6">
          ...
          <div className="grid md:grid-cols-3 gap-3">
            <div className="note">Item subtotal: <strong>${sub} CAD</strong></div>
            <div className="note">Shipping: <strong>${shipCost} CAD</strong></div>
            <div className="note total">Order total: ${total} CAD</div>
          </div>
          <p className="text-sm text-gray-600 mt-2">Examples — Standard: 1 candle $44, 2 $74, 3 $104 · Express: 1 $53, 2 $83, 3 $113</p>
        </section>
        ...
        <section className="mb-6">
          ...
          <div className="grid gap-2">
            <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-xl">
              ...
              <span>Interac e‑transfer — send to <strong>mutabazisabine27@gmail.com</strong></span>
            </label>
            <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-xl">
              <input type="radio" name="pay" onChange={()=>setPay("paypal")} checked={pay==="paypal"} />
              <span>PayPal — send to <strong>nemutv11@gmail.com</strong></span>
            </label>
          </div>
        </section>
        ...
        <div className="mb-4">
          <label className="text-sm text-gray-700"><input type="checkbox" required className="mr-2" /> I confirm my order details are correct.</label>
        </div>

        <button disabled={!canSubmit || loading} className="btn">{loading ? "Placing order…" : "Place order"}</button>
