...
export async function POST(req: Request){
  try{
    ...
    // Recompute server-side (authoritative)
    ...
    if(recomputedSubtotal !== data.itemSubtotal || recomputedTotal !== data.orderTotal){
      return NextResponse.json({
        ...
        details: { recomputedSubtotal, recomputedTotal, received: {
          itemSubtotal: data.itemSubtotal, orderTotal: data.orderTotal
        }}
      }, { status: 400 });
    }
    ...
    // Try emails; don’t fail the order if email fails
