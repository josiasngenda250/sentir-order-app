...
export async function POST(req: Request) {
  try {
    ...
    // Send emails but don't block order save if they fail
    let customerEmailError: string | null = null;
    let adminEmailError: string | null = null;

    try {
      await sendCustomerEmail(data.email, orderHtml);
    } catch (e: any) {
      console.error("sendCustomerEmail failed:", e?.message || e);
    ...
    try {
      ...
      console.error("sendAdminEmail failed:", e?.message || e);
